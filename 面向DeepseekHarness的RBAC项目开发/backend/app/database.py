...
职责边界：
  ...
  - 权限计算逻辑统一放在 services/rbac.py，鉴权依赖放在 deps.py。
"""
...
# 可移植的时间戳默认值
# ---------------------------------------------------------------------------
# 为什么不用 func.now()：SQLAlchemy 2.0 会把 func.now() 当成「表达式默认值」渲染，
# 在 MySQL 上生成 `DEFAULT (now())` —— 这是 MySQL 8.0.13+ 才支持的语法，
# 5.7 会直接报语法错误。写成 TEXT 形式则渲染为 `DEFAULT CURRENT_TIMESTAMP`，
# 5.7 与 8.0 都能接受，可移植性更好。
# 用法：  created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False,
...
class Base(DeclarativeBase):
    ...
    SQLAlchemy 2.0 风格：用 DeclarativeBase 子类代替旧的 declarative_base()。
    """
...
# pool_recycle=3600：每小时主动回收，比 wait_timeout 更保守。
...
def get_db() -> Generator[Session, None, None]:
    ...
    注意：这里「不」做 commit —— 写操作由各 service / router 显式提交，
    保证事务边界清晰（尤其是 R3 删除角色这种需要先查后写的场景）。
    """
    db = SessionLocal()
    try:
        yield db
...
def ensure_database_exists() -> bool:
    ...
    为什么要单独走一次无库名连接：MySQL 不允许在 USE 之前 CREATE DATABASE，
    而 SQLAlchemy 的 engine 已经绑定了库名，所以必须用 server_url 另开连接。
    """
...
def create_all_tables() -> None:
    ...
    # 必须先 import 模型模块，否则 Base.metadata 里没有任何表。
    from app import models  # noqa: F401  （副作用导入）
